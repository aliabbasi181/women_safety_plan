    ...
    if (_userSelectedContacts!.isNotEmpty) {
      for (Contact c in _userSelectedContacts!) {
        ...
        if (c.phones!.isNotEmpty) {
          String refactoredNumber = refactorPhoneNumbers(c.phones!.first.value.toString());
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: TextField(
          ...
          onChanged: (string) {
            setState(() {
              filteredContacts = _contacts!.where((c) => (c.displayName.toString().toLowerCase().contains(string.toLowerCase()))).toList();
            });
          },
        ),
      ),
      body: _contacts != null
          ...
          : Center(
              child: CircularProgressIndicator(valueColor: new AlwaysStoppedAnimation<Color>(Color(0xffbe3a5a))),
            ),
    );
  }
        ...
        : WidgetAnimator(
            Card(
              child: ListTile(
                ...
                leading: CircleAvatar(backgroundColor: Color(0xffbe3a5a), child: Text('${widget.c.displayName![0]}'.toUpperCase(), style: TextStyle(color: Colors.white)), radius: height * 0.025),
                title: Column(
